...
if [[ ! "$ver" = "" && ! "$ver" = "staging" && ! "$ver" = "production" ]]; then
    ...
    PACKAGE_VERSION=$(mvn -q help:evaluate -Dexpression=project.version -DforceStdout=true)
    if [[ ! "$PACKAGE_VERSION" = "" ]]; then
        echo "Auto detect the project version from package.json file and we'll use the version number: $PACKAGE_VERSION"
